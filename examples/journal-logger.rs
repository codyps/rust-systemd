...
fn main() {
    ...
    let mut reader =
        Journal::open(JournalFiles::All, runtime_only, local_only).expect("Could not open journal");
    ...
    reader
        .watch_all_elements(|record: JournalRecord| {
            let unit = record.get(KEY_UNIT).ok_or(Error::new(
                ErrorKind::Other,
                "Could not get unit from record",
            ))?;
            let message = record.get(KEY_MESSAGE).ok_or(Error::new(
                ErrorKind::Other,
                "Could not get message from record",
            ))?;
